...
export type ManagerNotification = {
  ...
  notificationType: ManagerNotificationType;
  title: string;
  message: string;
...
export async function createManagerNotification(
  ...
  const insertResult = await supabase.from("manager_notifications").insert({
  ...
  if (insertResult.error) {
    if (isMissingTableError(insertResult.error, "manager_notifications")) {
...
export async function loadManagerNotifications(
  supabase: SupabaseClient,
  options?: { limit?: number; unreadOnly?: boolean },
): Promise<{ notifications: ManagerNotification[]; unavailable: boolean }> {
  ...
  if (result.error) {
    if (isMissingTableError(result.error, "manager_notifications")) {
